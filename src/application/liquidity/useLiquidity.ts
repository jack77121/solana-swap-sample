...
const useLiquidity = create<LiquidityStore>((set, get) => ({
  ...
  findLiquidityInfoByTokenMint: async (
    ...
    console.log('in findLiquidityInfoByTokenMint, mint1: ', mint1, ' mint2: ', mint2);
    const availables = get().jsonInfos.filter(
      (info) =>
        (info.baseMint === mint1 && info.quoteMint === mint2) ||
        (info.baseMint === mint2 && info.quoteMint === mint1)
    );
    console.log('available vault: ', availables);
    ...
    const routeRelated = get().jsonInfos.filter((info) => {
      const isCandidate =
        candidateTokenMints.includes(info.baseMint) && candidateTokenMints.includes(info.quoteMint);
    ...
    const best = await (async () => {
      ...
      const largest = sameLevels.reduce((acc, curr) => {
        ...
        return gte(div(acc.lpSupply, 10 ** acc.lpDecimals), div(curr.lpSupply, 10 ** curr.lpDecimals))
          ? acc
          : curr;
      });
